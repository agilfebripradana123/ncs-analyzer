...
export default function Sidebar({ items }) {
  ...
  return (
    <>
      ...
      <aside
        className={`w-60 bg-sidebar h-screen fixed left-0 top-0 z-40 flex flex-col transition-transform ${
          open ? 'translate-x-0' : '-translate-x-full'
        } md:translate-x-0`}
      >
        <div className="p-6 border-b border-white/10">
          <h1 className="text-lg font-bold text-white">NCS Analyzer</h1>
          <p className="text-xs text-white/60 mt-0.5">Security Assessment</p>
        </div>

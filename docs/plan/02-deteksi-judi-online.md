...
Tanggal: 2026-09-25
Status: DONE (48 tests pass, build OK)

## Tujuan
Deteksi aktivitas judi online dari OCR text layar HP dan log aktivitas. Hybrid: keyword match cepat → TF-IDF cosine similarity fallback pakai dataset `judi.csv` (899 baris, label 1 = judi).
...
- **Anti-noise similarity:**
  - query < 2 term atau overlap < 2 term dengan corpus → batal match (fragment OCR 1 kata seperti "YouTube", "Super", "BS" tidak jadi finding)
  - URL di-strip sebelum tokenisasi (`https?://`, `www.`) — cegah boilerplate `www`+`com` dari YouTube link overlap corpus `www.sultanbet777.com` (Plan 05)
- **Dedup:** per frame simpan 1 similarity terbaik; lintas frame skip matched_corpus / analyzed_text sama dalam 60 detik
- **Integrasi:** langsung di `ProcessFrameDetections` (keyword dulu → similarity fallback) dan `ActivityParserService` untuk Layer 2
...
3. `app/Services/JudiSimilarityService.php` — normalize → strip URL → tokenize → TF-IDF → cosine → threshold
...
6. `app/Services/ActivityParserService.php` — sama: similarity rule dibaca dari DB (`rule_config->matcher = similarity`), threshold per-rule; `extractWebText` gabung title+url (titleUrl fallback), reparse idempoten (Plan 05)
7. `app/Services/JudiSimilarityService.php` — guard `MIN_QUERY_TERMS=2` + `MIN_OVERLAP_TERMS=2` + URL strip; evidence frame/log simpan `analyzed_text` + `matched_corpus`
...
## Verifikasi
- Unit test similarity service (termasuk anti-noise: `single common word does not match`, `single shared term overlap does not match`, `url only text does not match`)
